"""
Improved Training script for ViT-based Korean Food Classifier
...
def create_data_loaders(dataset_dir, batch_size=32, train_split=0.8, img_size=224):
    ...
    # IMPROVED: Much stronger data augmentation for ViT
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train ViT classifier with anti-overfitting techniques')
    ...
    parser.add_argument('--weight-decay', type=float, default=0.05, help='Weight decay (ViT typically uses higher)')
    ...
    parser.add_argument('--freeze-epochs', type=int, default=1, help='Number of epochs to train with frozen backbone')
    parser.add_argument('--no-freeze', action='store_true', help='Disable freezing backbone')
    parser.add_argument('--warmup-epochs', type=int, default=3, help='Number of warmup epochs')
    
    args = parser.parse_args()
    
    print("=" * 70)
    print(f"Training ViT Classifier ({args.model_type}) with Anti-Overfitting")
    print("=" * 70)
    ...
    print(f"  ✓ Cosine annealing scheduler with warmup ({args.warmup_epochs} epochs)")
    ...
    # IMPROVED: Freeze backbone for initial training
    if not args.no_freeze and args.freeze_epochs > 0:
        model.freeze_backbone()
        print(f"\n🔒 Stage 1: Training only classifier head for {args.freeze_epochs} epochs")
    ...
    # IMPROVED: Use AdamW with higher weight decay for ViT
    ...
    # IMPROVED: Cosine annealing with warmup (important for ViT)
    ...
    for epoch in range(start_epoch, args.epochs):
        # IMPROVED: Unfreeze after freeze_epochs
